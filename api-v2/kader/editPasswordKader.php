...
$getkader = mysqli_query($conn, "SELECT * FROM tb_auth WHERE user_kode = '$id_kader' AND role = 'Kader' ");
$data = mysqli_fetch_assoc($getkader);

if (password_verify($password_lama, $data["password"])) {
	$password = password_hash($password_baru, PASSWORD_DEFAULT);
    $query =  mysqli_query($conn, "UPDATE tb_auth SET password = '$password' WHERE user_kode = '$id_kader' AND role = 'Kader' ");
    if ($query){
